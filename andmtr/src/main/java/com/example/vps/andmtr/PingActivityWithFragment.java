...
public class PingActivityWithFragment extends Activity implements PingFragment.TaskCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ping_with_fragment);

        // get initial values from intent
        Intent intent = getIntent();
        // item string array: {"item name", "item address"}
        item = intent.getStringArrayExtra("item");
        item_address = item[1];
        // test string array: {"test name", "test type", "test interval", "test count", "test resolve"}
        test = intent.getStringArrayExtra("test");
        ping_wait_time = test[2];
        ping_interval = Integer.valueOf(ping_wait_time);
        ping_count = Integer.valueOf(test[3]);

                // find view elements
        edit = (EditText)findViewById(R.id.editText1);
        edit.setText(item_address);
    ...
    public void doPing(){
        if ( ( ping_counter < ping_count  &&  is_ping_started == 1 ) || ( ping_count == 0 && is_ping_started == 1 )) {
            ping_counter++;
            String host = String.valueOf(edit.getText());
    ...
    public void calculateStats(String input_lines) {
        ...
        // if first line starts with ERROR:
        ...
        pattern= Pattern.compile("^.+ping statistics ---");
        if ( pattern.matcher(line).matches() ){
